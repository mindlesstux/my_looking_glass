...
def kick_off_ping(uuid_str = None, dst_location=None, src_location="NopeDIEOUT", ip_family_ipv4 = False, ip_family_ipv6 = False, ping_count=10, redirect_to_null=False):
    ...
    cmd = "python3 %s/lg_cmd_ping.py --uuid=%s %s --count=%s %s %s %s" % (config.config['BIN_PATH'], uuid_str, ip_family, ping_count, src_location, dst_location, redirect)
    print("          CMD: %s" %(cmd))
